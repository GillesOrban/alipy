...
def affineremap(filepath, transform, shape, alifilepath=None, outdir = "alipy_out", makepng=False, hdu=0, verbose=True):
    ...
    data = scipy.ndimage.interpolation.affine_transform(data, matrix,
                                                        offset=offset,
                                                        output_shape = shape)
    ...
    if makepng:
        ...
        if not os.path.isdir(outdir):
            os.makedirs(outdir)
        myimage.tonet(os.path.join(outdir,
...
def irafalign(filepath, uknstarlist, refstarlist, shape,
    ...
    geomap = open(geomapinpath, "w")  # b needed for csv - GOX : 'b' removed...
