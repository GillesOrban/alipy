...
def _check_files(conf_file, conf_args, verbose=True):
    ...
    if conf_file is None:
        ...
        conf_file = '.pysex.sex'
    if not  'FILTER_NAME' in list(conf_args.keys()) or not os.path.isfile(conf_args['FILTER_NAME']):
        if verbose:
            print('No filter file found, using default filter')
        shutil.copyfile(default_conv, '.pysex.conv')
        conf_args['FILTER_NAME'] = '.pysex.conv'
    if not 'STARNNW_NAME' in list(conf_args.keys()) or not os.path.isfile(conf_args['STARNNW_NAME']):
        if verbose:
